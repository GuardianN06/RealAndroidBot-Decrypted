...
class LocalNetworkHandler(object):
    ...
    def decode_raw(self,data):
        ...
        for each_incense_pokemon in raw_incense_pokemon:
            ...
            if 'INCENSE_ENCOUNTER_AVAILABLE' in incense_pokemon_json.get('result','INCENSE_ENCOUNTER_UNKNOWN'):
                if self.check_duplicate(self.incense_pokemon,'encounterId',incense_pokemon_json.get('encounterId')) < 0: # Wild will include those from GMO
                    #pass
                ...
                for eaInventoryItem in inventory_data_json['inventoryDelta'].get('inventoryItem',[]):
                    if eaInventoryItem.get('inventoryItemData'):
                        if eaInventoryItem['inventoryItemData'].get('item'):
                            self.items[eaInventoryItem['inventoryItemData']['item'].get('itemId')] = eaInventoryItem['inventoryItemData']['item'].get('count',0)
            
            # Every round of holo inventory, add up all the balls
            self.total_ball_count = self.items.get('ITEM_POKE_BALL',0) + self.items.get('ITEM_GREAT_BALL',0) + self.items.get('ITEM_ULTRA_BALL',0) 
                    ...
                    if 'wildPokemon' in mapcell:
                        for eachWildPoke in mapcell['wildPokemon']:
                            ...
                            if int(despawntime) >= 90000:
                                if self.check_duplicate(self.wild,'encounterId',eachWildPoke.get('encounterId')) < 0: # Wild will include those from GMO
                                    if self.check_duplicate(self.rejected,'encounterId',eachWildPoke.get('encounterId')) < 0: # Make sure it's not in rejected list
                    ...
                    if 'nearbyPokemon' in mapcell and self.config['catch'].get('mon_to_chase',[]) and not self.chase_after_poke:
                        for nearbyPoke in mapcell['nearbyPokemon']:
                            ...
                            if self.check_if_in_list(self.config['catch'].get('mon_to_chase',[]), pokemon_name):
                                if self.check_duplicate(self.rejected_chase,'encounterId',nearbyPoke.get('encounterId')) < 0: # Make sure it's not in rejected list
        ...
        for encounter in raw_encounters:
            ...
            if encounter_json.get('pokemon'):
                if encounter_json['pokemon'].get('pokemon'):
                    ...
                    # Check not in wild
                    if self.check_duplicate(self.wild,'encounterId',encounter_json['pokemon'].get('encounterId')) < 0: # Wild will include those from GMO
                        if 'shiny' in encounter_json['pokemon']['pokemon'].get('pokemonDisplay'):
                            self.wild.insert(0,encounter_json.get('pokemon'))
                        else:
                            if self.check_duplicate(self.rejected,'encounterId',encounter_json['pokemon'].get('encounterId')) < 0: # Make sure it's not in rejected list
                                if despawntime >= 90000:
                                    self.wild.append(encounter_json.get('pokemon'))
                    else:
                        logger.debug('Encounter Detail: {}\n'.format(encounter_json.get('pokemon')))
                        encounterIndex = self.check_duplicate(self.encounter,'encounterId',encounter_json['pokemon'].get('encounterId'))
                        if encounterIndex < 0:
                    ...
                    if 'shiny' in encounter_json['pokemon']['pokemon'].get('pokemonDisplay') and self.config['catch'].get('go_after_shiny',True):
                        ...
                        logger.info('Wild Count: {} | Encounter Count: {} SHINY {} at ({},{})'.format(len(self.wild),len(self.encounter),encounter_json['pokemon']['pokemon'].get('pokemonId'), encounter_json['pokemon'].get('latitude'),encounter_json['pokemon'].get('longitude')))
                        ...
                        self.priorityList.append(encounter_json['pokemon'])
                        logger.info('Wild Count: {} | Encounter Count: {} 100IV {} at ({},{})'.format(len(self.wild),len(self.encounter),encounter_json['pokemon']['pokemon'].get('pokemonId'), encounter_json['pokemon'].get('latitude'),encounter_json['pokemon'].get('longitude')))
                        logger.debug('Wild Encounter: {}\n'.format(encounter_json.get('pokemon')))
                    else:
    ...
    def recv_size(self, the_socket):
        ...
        size_data=sock_data='';recv_size=SOCKSIZE
    ...
    def start_connection(self, host, port, recv_type='end'):
        ...
        while True:
            if recv_type=='size': result=self.recv_size(self.conn)
            elif recv_type=='end': result=self.recv_end(self.conn)
            elif recv_type=='timeout': result=self.recv_timeout(self.conn)
            else: result=self.conn.recv(SOCKSIZE) 
            try:
                self.decode_raw(json.loads(result.decode("utf-8")))
            except Exception as e:
                logger.error("Error Pharsing data: {}".format(e)) # Change exception to error for depolyment (so client wont see chuck of erros during exit)
                continue # Skip
