...
from page_detection import is_home_page, is_gym_page, is_pokestop_page, is_catch_pokemon_page, is_team_rocket_page, \
    ...
    is_profile_page, is_pokemon_full, is_pokestop_scan_page, is_not_pokestop_gym_on_map, is_exit_trainer_dialog, \
...
class Main:
    ...
    async def setup(self):
        ...
        try:
            ...
            logger.exception("An error occured while trying to get your device(s): {}".format(e)) # Change exception to error for depolyment (so client wont see chuck of erros during exit)
        ...
        if not self.config['client'].get('client','').lower() in ['polygon farmer', 'polygonfarmer']:
            ...
            if client:
                try:
                    ...
                    if not is_vaild:
                        if is_vaild > 0:
                            logger.info(f'You have use {is_vaild}/3 devices. Device list will reset when donation status is renewed.')
                        else:
        ...
        try:
            with session_scope() as session:
                first_login, last_login = device_checkin(session, full_device_info.get('serial',self.d.serial), self.config['client'].get('client','None'))
        ...
        if self.config['client'].get('lower_resolution',False):
            ...
            if is_home_page(im_rgb):
                if webhook_url and self.config['discord'].get('enabled',False):
                    send_to_discord(webhook_url, 'RAB Bot ({})'.format(self.device_id), 'Pokemon Go restarted. Resume scanning...')
            ...
            for y in range(1260, 1180, -10):
                ...
                if not is_home_page(im_rgb):
                    try:
                        ...
                        logger.exception("Encounter unexpected error: {}".format(e)) # Change exception to error for depolyment (so client wont see chuck of erros during exit)
                        cleanup()
                    break
            pgsharp_client.current_index += 1
        ...
        if self.config['client'].get('client', '').lower() in ['hal', 'pokemod']:
            ...
            if Unknown.is_not(self.pokemon.atk_iv) or \
                ...
                if self.config['discord'].get('notify_all_encountered',False) and self.config['discord'].get('enabled',False):
                    ...
                    message = '{}: {} Encountered. CP: {} ({}/{}/{}) Shiny: {}'.format(str_now, self.pokemon.name, self.pokemon.cp, self.pokemon.atk_iv, self.pokemon.def_iv, self.pokemon.sta_iv, self.pokemon.shiny)
                    webhook_url = self.config['discord'].get('webhook_url','') 
    ...
    async def check_and_send(self, pokemon, pgsharp_client=None):
        if pokemon.iv == 100:
            try:
                ...
                text = "{} {} CP{} {}/{}/{} level{} {},{}".format(pokemon.name, pokemon.gender, pokemon.cp, pokemon.atk_iv, pokemon.def_iv, pokemon.sta_iv, pokemon.level, pokemon.latitude, pokemon.longitude)
        ...
        if self.config['client'].get('auto_goplus',False):
            ...
            while True:
                ...
                if not is_plus_disconnected(im_rgb, offset):
                    ...
                    await tap_screen(self.p, 990, 450 + offset, 1.0)
    ...
    async def snipe(self):
        ...
        if len(spawns_to_snipe) > 0:
            ...
            if is_catch_pokemon_page(im_rgb):
                ...
                logger.info('Snipe Pokemon not found. Pausing for {:.2f} mins before restarting route...'.format(cd_total_sec/ 60))
                await asyncio.sleep(cd_total_sec)
                logger.info('Resuming route...')
                await self.p.start_route(self.config['snipe']['default_route_name'])
                if self.config['client'].get('auto_goplus',False):
    ...
    async def shiny_check(self):
        ...
        if self.pgsharpv2:
            ...
            # all okay, let's move
            try:
                x, y = await pgsharp_client.get_item_position(self.d(resourceId='me.underworld.helaplugin:id/hl_sri_icon', packageName='com.nianticlabs.pokemongo')[0].info, self.config.get('resize',False)) # Let's try always tap first one
            except:
                return 'on_error'
    ...
    async def farmer(self):
        ...
        if len(localnetwork.catch) > 0 and len(localnetwork.encounter) > 0:
            ...
            if caught_status.get('status') == 'CATCH_SUCCESS':
                ...
                logger.info('{}: {} Caught CP: {} ({}/{}/{}) Shiny: {}'.format(str_now, self.pokemon.name, self.pokemon.cp, self.pokemon.atk_iv, self.pokemon.def_iv, self.pokemon.sta_iv, self.pokemon.shiny))
                message = '{}: {} Caught CP: {} ({}/{}/{}) Shiny: {}'.format(str_now, self.pokemon.name, self.pokemon.cp, self.pokemon.atk_iv, self.pokemon.def_iv, self.pokemon.sta_iv, self.pokemon.shiny)
                ...
                message += '\nTotal Experience: {} | Candies: {} | XL: {} | Stardust: {}'.format(ttl_exp, ttl_candies, ttl_XL, ttl_stardust)
                ...
                message = '{}: {} CP: {} ({}/{}/{}) Shiny: {} FLED'.format(str_now, self.pokemon.name, self.pokemon.cp, self.pokemon.atk_iv, self.pokemon.def_iv, self.pokemon.sta_iv, self.pokemon.shiny)
            else:
                logger.error("Error...")
            localnetwork.encounter[:] = []
            localnetwork.wild[:] = []
    ...
    async def polygon(self):
        ...
        if localnetwork.slotable_gym and self.poke_in_gym < 9 and self.config['client'].get('auto_slot', False) and not self.config['catch'].get('mon_to_chase',False):
            ...
            logger.info('Moving to Gym for sloting (Availible Slots: {}) Cooldown: {}'.format(slotableGym['gymDisplay'].get('slotsAvailable'),default_CD))
            ...
            await tap_screen(self.p, poke_location[self.poke_in_gym].get('x'), poke_location[self.poke_in_gym].get('y'), 2) # First Pokemon in list position
        ...
        if ((len(localnetwork.wild)>0 or len(localnetwork.encounter)>0) and localnetwork.mode == 1) or localnetwork.priorityList or localnetwork.incense_pokemon or localnetwork.chase_after_poke:
            logger.debug('wild: {} | encounter: {} | incense: {} | mode: {}'.format(len(localnetwork.wild),len(localnetwork.encounter), len(localnetwork.incense_pokemon), localnetwork.mode))
            if not localnetwork.priorityList and not localnetwork.incense_pokemon and not localnetwork.chase_after_poke:
                if not localnetwork.inventory_full:
                    if ((localnetwork.items and localnetwork.total_ball_count <= self.config['catch'].get('stop_at_ball',10) and not localnetwork.priorityList)) or localnetwork.total_ball_count == 0:
                        ...
                        localnetwork.priorityList[:] = [] # got stuck after catching pokemon. added this to see if this the problem
                        localnetwork.sort_pokestops() # sort pokestop here everytime mode change. this will make RAB go after the nearest pokestops instead of jumping large distance
                        localnetwork.mode = 0
                        return
            ...
            if localnetwork.chase_after_poke:
                ...
                if poke_to_chase.get('latitude',0) != 0:
                    ...
                    default_CD = calculate_cooldown(last_active_location['latitude'], 
                                                    ...
                                                    lat, 
                                                    lng)
                    logger.info('Chasing after {} at {},{} CD: {}'.format(POKEMON[poke_to_chase.get('pokedexNumber',0)], lat, lng, default_CD))
            ...
            if not localnetwork.priorityList and not localnetwork.incense_pokemon:
                if poke_to_chase:
                    ...
                    for each_wild in for_sort:
                        logger.debug('WILD: {} CHASE: {}'.format(each_wild['pokemon'].get('pokemonId'),POKEMON[poke_to_chase.get('pokedexNumber')].replace(' ','_').upper()))
            ...
            while True:
                    ...
                    if encounterIndex >= 0 or poke_found:
                        ...
                        if is_catch_pokemon_page(im_rgb):
                            ...
                            if newEncounterIndex >= 0:
                                localnetwork.wild.pop(newEncounterIndex) # remove from wild poke so that bot will not go after it anymore
                            localnetwork.repeat_encounter[:] = []
                ...
                if is_catch_pokemon_page(im_rgb):
                    ...
                    if self.config['catch'].get('only_shiny', False) and not self.pokemon.shiny:
                        ...
                        localnetwork.encounter[:] = []
                        return 'on_pokemon'
                    pokemon_caught = await catch_pokemon(self.p, self.d, self.pokemon, rab_runtime_status = rab_runtime_status, device_id = self.device_id) # Catch it the normal way
                    if pokemon_caught == 'No Ball':
                        localnetwork.total_ball_count = 0
                        localnetwork.mode = 0
                    if pokemon_caught and not self.config['client'].get('transfer_on_catch',False):
        ...
        if len(localnetwork.pokestop) > 0 and (len(localnetwork.wild) == 0 or localnetwork.mode == 0): # spin mode
            ...
            if is_catch_pokemon_page(im_rgb):
                ...
                if self.config['catch'].get('only_shiny', False) and not self.pokemon.shiny:
                    ...
                    localnetwork.encounter[:] = []
                    return 'on_pokemon'
                pokemon_caught = await catch_pokemon(self.p, self.d, self.pokemon,localnetwork, rab_runtime_status = rab_runtime_status, device_id = self.device_id) # Catch it the normal way
                if pokemon_caught == 'No Ball':
                    localnetwork.total_ball_count = 0
                    localnetwork.mode = 0
                if pokemon_caught and not self.config['client'].get('transfer_on_catch',False):
                    ...
                    if is_catch_pokemon_page(im_rgb, is_shadow=True):
                        ...
                        if (pokemon_caught and self.config['client'].get('transfer_on_catch',False)):
                            if pokemon_caught != 'No Ball':
                                ...
                                await fav_last_caught(self.p, self.d, self.pokemon)
                                await asyncio.sleep(1)
                        localnetwork.incident[:] = [] # clear this list as this is likely triggered due to data curruption/slow network reply
            ...
            with session2_scope() as session:
                ...
                if total_forts > 0:
                    for each_fort in fort_details:
                        logger.info(f"Going to ID: {each_fort.id} Name: {each_fort.name} coord: {each_fort.lat},{each_fort.lon}")
                            ...
                            if len(localnetwork.gym) > 0:
                                ...
                                if round(each_fort.lat,5) == round(network_lat,5) and round(each_fort.lon,5) == round(network_lon,5):
                                    ...
                                    with session2_scope() as session2:
                                        update_forts(session2, each_fort_id, gym_details.get('name','UNKOWN'), gym_details.get('url','').replace('http','https'))
                                        logger.info("Updated: {}".format(gym_details.get('name','UNKOWN')))
                                        summary_gym.append(gym_details.get('name','UNKOWN'))
                                else:
                                    logger.info("Incorrect gym: {} should be: {}".format(gym_details.get('name','UNKOWN'), each_fort.name))
            ...
            with session2_scope() as session:
                ...
                if total_pokestops > 0:
                    for each_pokestop in pokestop_details:
                        logger.info(f"Going to ID: {each_pokestop.id} Name: {each_pokestop.name} coord: {each_pokestop.lat},{each_pokestop.lon}")
                        ...
                        if await load_spawns(self.p, self.d, target):
                            ...
                            if len(localnetwork.fort) > 0:
                                ...
                                if round(each_pokestop.lat,5) == round(network_lat,5) and round(each_pokestop.lon,5) == round(network_lon,5):
                                    ...
                                    with session2_scope() as session2:
                                        update_pokestops(session2, each_pokestop_id, pokestop_details.get('name','UNKOWN'), pokestop_details['imageUrl'][0].replace('http','https'))
    ...
    async def reset_app(self):
        ...
        if 'polygon' in self.config['client'].get('client','').lower():
            ...
            if not pid:
                ...
                if webhook_url and self.config['discord'].get('enabled',False):
                    send_to_discord(webhook_url, 'RAB Bot ({})'.format(self.device_id), 'An Error has occoured while trying to start Polygon Sharp')
                cleanup()
            else:
        ...
        if 'hal' in self.config['client'].get('client','').lower():
            ...
            if not pid:
                ...
                if webhook_url and self.config['discord'].get('enabled',False):
                    send_to_discord(webhook_url, 'RAB Bot ({})'.format(self.device_id), 'An Error has occoured while trying to start Pokemod HAL')
                cleanup()
            else:
        ...
        if 'pokemod' in self.config['client'].get('client','').lower():
            ...
            if not pid:
                ...
                if webhook_url and self.config['discord'].get('enabled',False):
                    send_to_discord(webhook_url, 'RAB Bot ({})'.format(self.device_id), 'An Error has occoured while trying to start Pokemod Espresso')
                cleanup()
            else:
        ...
        if 'mad' in self.config['client'].get('client','').lower():
            ...
            if not pid:
                ...
                if webhook_url and self.config['discord'].get('enabled',False):
                    send_to_discord(webhook_url, 'RAB Bot ({})'.format(self.device_id), 'An Error has occoured while trying to start MAD Enhancer')
                cleanup()
            else:
    ...
    async def start(self):
        ...
        if self.map_mode:
            ...
            localnetwork = LocalNetworkHandler(self.config['network'].get('host','0.0.0.0'),self.config['network'].get('port',5120))
        ...
        if self.config['client'].get('client','').lower() in ['pgsharp', 'pgsharp paid', 'pgsharppaid']:
            ...
            if (int(time.time()) <= self.limited_time_feature) or self.subscriber:
                ...
                if  frame_count >= 2:
                    self.pgsharpv2 = False
                    logger.info("2 or more overlay frame detected. Use Nearby Radar for Enchanced PGSharp feature of RAB. Use Quick Sniper for Shuno hunting.")
                elif frame_count == 1:
                    if self.config['client'].get('pgsharp_shuno_hunt',0):
                        ...
                        if not self.subscriber:
                            logger.info("PGSharp 100IV Hunting Mode is a limited time trial feature and will expire on: {}".format(datetime.fromtimestamp(self.limited_time_feature).strftime("%d %b %Y, %H:%M:%S")))
                        ...
                        logger.info("Note: Enchanced PGSharp feature of RAB might not work for all phones, if it doesn't work for you, disable Nearby Radar and fall back to normal mode.")
                        #if not self.subscriber:
                    ...
                    if self.config['client'].get('pgsharp_shuno_hunt',0):
                        logger.info("No overlay frame detected for PGSharp. Please enable Quick sniper to enable PGSharp 100IV Hunting feature of RAB.")
                    else:
                        logger.info("No overlay frame detected for PGSharp. Please enable Nearby Radar to enable Enchanced PGSharp feature of RAB.")
                    self.pgsharpv2 = False
            else:
                pgsharp_client = PGSharp()
                logger.info("Please wait.... Checking and readjusting icons...")
                ...
                if  frame_count >= 2:
                    ...
                    if self.config['client'].get('pgsharp_shuno_hunt',0):
                        ...
                        logger.info("No overlay frame detected for PGSharp. Enable Nearby Radar to enable Enchanced PGSharp feature of RAB.")
                    self.pgsharpv2 = False
                        ...
                        if time_diff >= (30 * 60):
                            ...
                            if self.config['client'].get('client','').lower() == 'hal':   
                                ...
                                if is_plus_disconnected(im_rgb, offset):
                                    ...
                                    logger.info("Go Plus is still connecting. Attempt to disconnect and reconnect to prolong Go Plus connection...")
                                    await tap_screen(self.p, 990, 450 + offset, 1.0)
                                    i = 0
                                    while True:
                                        if i == 7:
                ...
                if (self.config['telegram'].get('enabled') and self.config['shiny_check'].get('enabled')) or (self.config['client'].get('pgsharp_shuno_hunt',0) and self.pgsharpv2):
                    ...
                    if result == 'not_shiny':
                        ...
                        if (self.no_spawn_count >= 3) or (self.no_spawn_count >= 2 and self.pgsharpv2) :
                            await self.reset_app()
                            logger.info("Restarting App because there are no spawn for detected for {} times".format(self.no_spawn_count))
                            webhook_url = self.config['discord'].get('webhook_url','')
                            self.d.app_wait("com.nianticlabs.pokemongo", front=True)
                            sleep_time = 60 if self.config['client'].get('client','').lower() != 'mad' else 90
                            logger.info(f'Pokemon Go App started, waiting for {sleep_time} secs to login...')
...
async def telegram_feed(event):
    try:
        ...
        to_id = int('-100' + str(raw_to_id.channel_id)) # Need to find out how to get the correct id for public channel, this only works for private
        ...
        if spawn.get('latitude') and spawn.get('longitude') and to_id in feed_src:
            ...
            if cd_total_sec <= config_max_cd and config_max_cd != 0  and isinstance(spawn['atk'], int) and isinstance(spawn['def'], int) and isinstance(spawn['sta'], int) and isinstance(spawn['level'], int):
                logger.debug("--- This Poke can be snipe within {} min: {}---\nRaw Data: {}".format(round(cd_total_sec/60,2),spawn,text))
                # let see if this is the pokemon that we really
                great_rating, great_id, great_cp, great_level, ultra_rating, ultra_id, ultra_cp, ultra_level = get_pvp_info(spawn.get('dex',1), spawn.get('atk',0), spawn.get('def',0), spawn.get('sta',0), spawn.get('level',1))
                for pokename, each_poke in config['snipe'].get('snipe_list', {}).items():
                    ...
                    if poke_found:
                        if snipe_count.get(pokename.lower(),0) >= (each_poke.get('snipe_limit',0) - 1) and each_poke.get('snipe_limit',0) !=0:
                            logger.info('Ignored {}, snipe limit {} reached'.format(spawn['name'], each_poke.get('snipe_limit',0)))
                            continue
        ...
        logger.exception("Encounter Telegram Error: {}".format(e)) # Change exception to error for depolyment (so client wont see chuck of erros during exit)
        pass
...
def call_main(event=None, telegram_client=None, frm_telegram_id=None, frm_donor_until=None):
    ...
    if client:    
        ...
        if frm_donor_until:
            feed_dict = {'Free Feed': -1001204900874, '100IV Shiny': -1001263837537, '82IV Shiny': -1001342429024, 'Rare 100IV': -1001474496106, 'PVP': -1001519876298, 'Hidden 1': -1001329595625, 'Hidden 2': -1001469656053}
        else:
            feed_dict = {'Free Feed': -1001204900874}
