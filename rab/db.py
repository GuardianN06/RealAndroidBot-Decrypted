...
engine = create_engine(config.DB_ENGINE, pool_size=config.DB_POOL_SIZE, max_overflow=config.DB_MAX_OVERFLOW, pool_recycle=config.DB_POOL_RECYCLE, pool_pre_ping=True)
engine2 = create_engine(config.DB_ENGINE2, pool_size=config.DB_POOL_SIZE, max_overflow=config.DB_MAX_OVERFLOW, pool_recycle=config.DB_POOL_RECYCLE, pool_pre_ping=True)
...
def get_empty_forts(session):
    try:
        fort_details = session.query(Forts).filter(or_(Forts.name=='UNKOWN',Forts.name==None, Forts.name=='',Forts.url=='UNKOWN',Forts.url==None, Forts.url=='')).all()
...
def get_empty_pokestops(session):
    try:
        pokestop_details = session.query(Pokestops).with_entities(Pokestops.id, Pokestops.external_id, Pokestops.lat, Pokestops.lon, Pokestops.name, Pokestops.url).filter(or_(Pokestops.name=='UNKOWN',Pokestops.name==None, Pokestops.name=='', Pokestops.url=='UNKOWN',Pokestops.url==None, Pokestops.url=='')).all()
